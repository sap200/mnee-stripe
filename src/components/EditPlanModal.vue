        ...
        <div class="form-row features-row">
          ...
          <div class="features-list">
            <div v-for="(feature, index) in form.features" :key="index" class="feature-input">
              <input v-model="form.features[index]" type="text" required placeholder="Feature" />
              <button
                type="button"
            ...
            <button type="button" class="btn small primary" @click="addFeature">
              + Add Feature
            </button>
          </div>
        </div>
...
export default {
    ...
    plan: {
      ...
      handler(newPlan) {
        this.form = {
          ...
          name: newPlan.name || "",
          code: newPlan.code || "",
          amount: newPlan.amount || 0,
          interval: newPlan.interval || "monthly",
          status: newPlan.status || "active",
          features: newPlan.features && newPlan.features.length ? [...newPlan.features] : [""],
        };
      },
    },
  },
  methods: {
    ...
    handleUpdate() {
      ...
      this.$emit("update-plan", { ...this.form, features: validFeatures });
      this.$emit("close");
    },
  },
};
